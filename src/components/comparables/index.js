        ...
        <div key = {movie.title} className='movies'>
          <div className='comparison-movie-display'>
            ...
            <div className='movie-title-wrapper'>
              <h1>{ movie.title } </h1>
              <h1>Release Date: { new Date(movie.us_theatrical_release).toLocaleDateString('en-US', {day : 'numeric', month : 'long', year : 'numeric'})}</h1>
              <h1>Total Box Office: ${(parseInt(movie.us_gross_bo) + parseInt(movie.intl_gross_bo)).toLocaleString()}</h1>
            </div>
          </div>
        </div>
      )
